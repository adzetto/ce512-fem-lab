...
\end{axis}
\end{tikzpicture}
\caption{Assembly and solve time vs.\ $n$ (Vectorized Assembly, CUDA GPU Solve for $n \ge 256$). Assembly scales as $O(n^2)$ (fill $n^2$ entries); the solve scales as $O(n^3)$ but highly offloads computation to the GPU.}
\label{fig:timing}
\end{figure}
...
Large-scale &
The $n\times n$ system was assembled without loops and solved up to $n=10{,}000$ using vectorization and CUDA GPU offloading. Assembly scales as $O(n^2)$, GPU solve as $O(n^3)$.  The Hilbert-like condition number $\kappa_2\sim 10^{22}$ prevents convergence beyond $n\approx 15$---motivating local FEM bases over global polynomials. \\
\bottomrule
\end{tabular}
